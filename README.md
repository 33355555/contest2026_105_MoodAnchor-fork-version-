...
1. **无感实时觉察**：围绕 IMU 运动过滤与皮电（GSR/EDA）生理确认，规划端侧疑似事件提示；不使用语义识别来判断对话内容。
2. **阻断冲动危机**：在疑似事件出现时，通过手机提醒与用户主动启用的音频播放，让用户获得短暂的注意力转移与离场理由。
...
| 级别 | 适用设想 | 干预方式 | 设计目的 |
| --- | --- | --- | --- |
| 初级：温和提醒 | 未连接耳机或处于公开社交场景 | 手机通知与界面提示 | 提供自主暂停与查看记录的入口 |
| 音频辅助 | 用户主动启用并选择本地音频 | 自动播放前检查蓝牙耳机连接状态；未检测到耳机则跳过 | 用用户熟悉的声音提供暂停提示 |
| App 陪伴 | 事件后或用户主动开启时 | 通知、记录、CBT 风格陪伴对话与小行动建议 | 把短暂的暂停延展为可回看的情绪调节过程 |

音频默认关闭，需用户主动启用并选择文件。App 在自动播放请求发出前检查蓝牙耳机连接状态，未检测到耳机时不启动播放，也不主动选择扬声器作为替代输出。播放后的实际输出由系统路由决定；当前不承诺耳机断连或输出切换后的路由行为。产品不提供医疗干预或强制行为控制。
...
- 黄山派端的 GSR、IMU、屏幕、BLE 等硬件接口集成与事件上报闭环。
- Android App 接收手表 BLE 事件、显示记录、通知提醒，并提供音频设置与陪伴对话入口。
- App 经腾讯云 SCF 调用 Coze 的在线对话链路；模型 API Key 保留在服务端，APK 仅内置服务地址与公开评审访问码。
- IMU+EDA 训练、评估、个体化流程与端侧 C 模型导出材料；模型材料与当前手表运行逻辑的边界如下。

手表端默认采用已在演示链路中验证的原型阈值事件逻辑，以保障采集、BLE 上报和手机协同稳定。`algorithm/imu_eda/` 提供 IMU 运动过滤与 EDA 生理确认的训练材料、端侧 C 串行门控核心和参数；在完成黄山派真实采样的采样率、窗口、特征顺序与单位对齐验证前，它不会被描述为默认的板端情绪识别实现。

App 中的 `0.74` 是固定演示分数，仅用于展示记录、通知和交互，不代表传感器实测概率。实际模型概率需由手表端完成算法部署、输入校准和验证后计算并上报；概率值本身不等于算法准确率。演示记录也不作为模型性能证据。

数据展示统一见 [IMU + EDA 数据集与指标](docs/ALGORITHM_METRICS.md)，仅展示 IMU 与 EDA 的独立模态结果。
...
尚未实现的产品愿景还包括**手表端马达震动提示**。当前自动音频／语音提示以蓝牙耳机已连接且用户主动启用为前提；未来计划通过腕部震动提供无需耳机、无需查看手机的提醒方式，期望比手机状态栏通知更直接、更迅捷地被用户感知。马达接入、驱动与触发逻辑尚未实现，其提醒时延和实际体验也尚未验证，不计入当前已完成功能。
...
当前评审链路为 Android App → 腾讯云 SCF Web 函数 → Coze。2026-09-18 已完成健康检查及手机实际对话验证；这是当次联调结果，不保证服务持续在线或模型额度永远充足。评审访问码随源码和 APK 公开，Coze API Key 仅保存在服务端。部署与维护见 [腾讯云说明](backend/tencent_scf/README.md)。Cloudflare 实现保留作备选，当前不作为评审入口。

如需自行部署本地中转服务，在仓库根目录使用 Python 3 设置所选模型平台的环境变量并启动：
