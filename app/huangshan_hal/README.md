...
`HS_BLE_EV_SELFTEST (0xff)` 事件（手机端把它显示为"自检 / 台架测试"），
flags 带 `SIMULATED | ACK_REQ`。
同一份模拟判定会接管数据包的情绪字节 10 秒，让手机端看到与事件一致的变化。

LINK 页会显示 `sim mood #N ...`，串口打印 `[sim] mood change #N: ...`，
手机端 App 会显示"收到手表事件：自检 / 台架测试（置信度 N%）"。

**事件类型只能用手机端已认识的值**（`0x01..0x06`、`0xff`）：接收端是固定
的、不能改，新类型在手机上只会显示"未知事件"。情绪结论本身走数据特征
byte 15（`agitated + confidence`），不依赖新增事件类型。
...
外接传感器为 GSR；IMU 使用板载接口。BLE Data 的字节 2、3 和 Status 的字节 4、5 保留原有偏移，当前数值为 0、对应有效位清零，不代表额外的测量能力。
